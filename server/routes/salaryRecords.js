...
router.get('/:id', async (req, res) => {
  try {
    const doc = await getDB().collection('salaryRecords').findOne({ _id: req.params.id, shopId: req.user.shopId });
...
router.delete('/:id', async (req, res) => {
  try {
    await getDB().collection('salaryRecords').deleteOne({ _id: req.params.id, shopId: req.user.shopId });
